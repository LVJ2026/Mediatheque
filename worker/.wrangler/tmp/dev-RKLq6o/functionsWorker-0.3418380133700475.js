...
// .wrangler/tmp/bundle-ch6Awf/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-pH5Nk6/functionsWorker-0.3418380133700475.mjs
...
__name(gristUrl, "gristUrl");
__name2(gristUrl, "gristUrl");
...
__name(gristRequest, "gristRequest");
__name2(gristRequest, "gristRequest");
...
__name(assertConfigured, "assertConfigured");
__name2(assertConfigured, "assertConfigured");
...
function addDays(value, amount) {
  return new Date((/* @__PURE__ */ new Date(`${value}T12:00:00`)).getTime() + amount * DAY_MS).toISOString().slice(0, 10);
}
__name(addDays, "addDays");
__name2(addDays, "addDays");
function normalizeGame(record) {
  ...
  return {
    ...
    "\xC2ge indiqu\xE9": fields["\xC2ge indiqu\xE9"] ?? fields.Age_indique ?? "",
    Joueurs: fields.Joueurs ?? "",
    Remarques: fields.Remarques ?? ""
  };
}
__name(normalizeGame, "normalizeGame");
__name2(normalizeGame, "normalizeGame");
function normalizeLoan(record) {
  ...
  return {
    id: record.id,
    name: fields.Nom || "",
    first_name: fields.Prenom || fields["Pr\xE9nom"] || "",
...
__name(normalizeLoan, "normalizeLoan");
__name2(normalizeLoan, "normalizeLoan");
async function onRequestPost({ request, env }) {
  try {
    ...
    if (conflict)
      return Response.json({ detail: "Au moins un jeu est d\xE9j\xE0 r\xE9serv\xE9 sur cette p\xE9riode." }, { status: 409 });
...
async function onRequestGet2({ env }) {
  try {
    ...
    if (error.message.startsWith("Grist 404"))
      return Response.json([]);
    return Response.json({ detail: error.message }, { status: 503 });
  }
}
...
// .wrangler/tmp/bundle-ch6Awf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ch6Awf/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.3418380133700475.js.map
